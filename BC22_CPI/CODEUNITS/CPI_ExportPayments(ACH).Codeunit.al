codeunit 50100 "Export Payments (ACH) CPI"
{
    ...
    begin
        ...
        //Added for CITI NA ACH
        //B021000089 if using a Citibank, N.A. Account for Settlement / b=blank
        CompanyInformation.TESTFIELD("Payment Routing No.");
        ...
        with BankAccount do begin
            ...
            if not ("Export Format" in ["Export Format"::US]) then
                TestField("Export Format", "Export Format"::US);
            //NPO end
            ...
            //NPO begin
            OnBeforeExportPrnString(BankAccount, FileHeaderRec, Justification, FileDate, FileTime, RecordLength, BlockingFactor, ReferenceCode, IsHandled);
    ...
    begin
        ...
        AddToPrnString(FileHeaderRec, CompanyInformation."Payment Routing No.", 4, 10, Justification::Right, ' ');//Immediate destination //Citibank N.A.
        ...
        // AddToPrnString(FileHeaderRec, 'A', 34, 1, Justification::Right, '0'); //PACT
    ...
    begin
        ...
        // //K12 begin
        // AddToPrnString(DetailRec, '0', 55, 4, Justification::Left, '0');
        // AddToPrnString(DetailRec, CompanyInformation.Name, 59, 16, Justification::Left, ' ');     // Company Name
        // AddToPrnString(DetailRec, ' ', 75, 2, Justification::Left, ' ');
        // //K12 end    
        AddToPrnString(DetailRec, '0000' + CompanyInformation.Name, 55, 22, Justification::Left, ' ');     // Company Name
    ...
    begin
        ...
        with GenJnlLine do begin
            ...
            AddToPrnString(DetailRec, '  ', 75, 2, Justification::Left, ' ');
            AcctType := ' '; //CPI 2020 Citibank NACHA                           // Reserved
        ...
        //CPI Added Addendum CITIBANK NA
        ExportAddendumRecord(GenJnlLine);
        exit(GenerateFullTraceNoCode(TraceNo));
    end;

    //NPO begin
    procedure ExportElectronicPayment(GenJnlLine: Record "Gen. Journal Line"; PaymentAmount: Decimal; pExport: Boolean): Code[30]
    var
        Vendor: Record Vendor;
        VendorBankAcct: Record "Vendor Bank Account";
        Customer: Record Customer;
    ...
    begin
        ...
        with GenJnlLine do begin
            ...
            if AcctType = 'V' then begin
                ...
                OnAfterAcctTypyV(SavingAccount, VendorBankAcct);
            end else
                if AcctType = 'C' then begin
                    Customer.Get(AcctNo);
                    if Customer."Privacy Blocked" then
                    ...
                    OnAfterAcctTypeC(SavingAccount, CustBankAcct);
                end;
            ...
            if SavingAccount then begin
                if DemandCredit then
                    AddNumToPrnString(DetailRec, 32, 2, 2)          // Transaction Code -> Demand Credit: Automated Deposit
                else
                    AddNumToPrnString(DetailRec, 37, 2, 2);         // Transaction Code -> Demand Debit: Automated Payment
            end else begin
                if DemandCredit then
    ...
    begin
        ...
        //CPI 2020 removed for CITIBANK NACHA
        ...
        ClientFile := BankAccount."Last E-Pay Export File Name";
#if not CLEAN17
        // RBMgt.DownloadToFile(FileName, ClientFile);
        DownloadFromStream(Instr, '', '', '', ClientFile);
    ...
    begin
        ...
        OnBeforeAddFedIDToPrnStringNVG(gCustomerDirectDebit, PrnString, FedID, StartPos, Length, IsHandled); //NPO 
        if IsHandled then
            exit;
    ...
    //NPO begin
    [IntegrationEvent(false, false)]
    local procedure OnBeforeExportPrnString(var BankAccount: Record "Bank Account"; var FileHeaderRec: Text[250]; Justification: Option; FileDate: Date;
    FileTime: Time; RecordLength: Integer; BlockingFactor: Integer; ReferenceCode: Code[10]; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeAddFedIDToPrnStringNVG(gCustomerDirectDebit: Boolean; var PrnString: Text[250]; FedID: Text[30]; StartPos: Integer; Length: Integer; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterAcctTypyV(var SavingAccount: Boolean; var VendorBankAcct: Record "Vendor Bank Account")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterAcctTypeC(var SavingAccount: Boolean; var CustBankAcct: Record "Customer Bank Account")
    begin
    end;

    procedure SetCustomerDirectDebit(pCustomerDirectDebit: Boolean);
    begin
        gCustomerDirectDebit := pCustomerDirectDebit;
    end;
    //NPO end
    procedure ExportAddendumRecord(GenJnlLine: Record "Gen. Journal Line"): Code[30]
    var
        DetailRec: Text[250];
    begin
        IF NOT FileIsInProcess THEN
        ...
        AddNumToPrnString(DetailRec, 7, 1, 1);          // Record Type Code
        AddNumToPrnString(DetailRec, 5, 2, 2);     // Addenda Type Code

        AddToPrnString(DetailRec, GenJnlLine.Description, 4, 80, Justification::Left, ' ');           // Payment Related information
        AddToPrnString(DetailRec, GenerateTraceNoCode(TraceNo), 84, 14, Justification::Left, ' ');       // Addenda Sequence Number
        // AddNumToPrnString(DetailRec,1,84,4);  // Addenda Sequence Number
